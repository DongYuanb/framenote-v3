...
try:
    ...
    AGNO_AVAILABLE = True
except ImportError as _e:  # 只捕获 ImportError
    logger.warning(f"agno 库导入失败，将使用占位实现: {_e}")
    Agent = object  # type: ignore
    TextKnowledgeBase = object  # type: ignore
    LanceDb = object  # type: ignore
    SearchType = object  # type: ignore
    OpenAIChat = object  # type: ignore
    CohereEmbedder = object  # type: ignore
...
class VideoNotesAgentService:
    ...
    embedder = CohereEmbedder() if AGNO_AVAILABLE else None

    def __init__(self):
        self.agent_sessions: Dict[str, Agent] = {}
        self.knowledge_bases: Dict[str, TextKnowledgeBase] = {}

    def _get_vector_db(self, task_id: str):
    ...
    def _load_notes_knowledge(self, task_id: str):
        """加载指定task_id的笔记知识库"""
        try:
            if not AGNO_AVAILABLE:
                return None
    ...
    def get_or_create_agent(self, task_id: str, user_id: str = "user"):
        """获取或创建指定task_id的agent实例"""
        agent_key = f"{task_id}_{user_id}"

        if agent_key not in self.agent_sessions:
            if not AGNO_AVAILABLE:
                # 降级的占位对象，提供最基本的 echo 能力
                class _DummyAgent:
                    def __init__(self):
                        self.knowledge = None
                    def run(self, message: str, stream: bool = False):
                        if stream:
                            for chunk in ["抱歉，智能 Agent 组件未安装，", "已切换占位实现，", "请先完成 agno 相关依赖安装后再试。"]:
                                yield {"content": chunk}
                            return
                        return type("Resp", (), {"content": "当前未安装智能 Agent 依赖（agno），已使用占位响应。"})()

                self.agent_sessions[agent_key] = _DummyAgent()
            else:
                # 加载笔记知识库
